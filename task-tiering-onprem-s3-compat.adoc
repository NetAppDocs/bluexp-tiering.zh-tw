...
* 安裝在內部部署上的 Connector 。
* 連接器的網路功能、可將HTTPS連線傳出至來源ONTAP 支援的叢集、S3相容的物件儲存設備、以及雲端分層服務。
...
+
** 透過連接埠 443 與雲端分層服務的 HTTPS 連線 (https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-set-up-networking-on-prem.html#endpoints-contacted-for-day-to-day-operations["請參閱端點清單"^]）
