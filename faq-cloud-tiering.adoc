...
支援的是可透過CLI和System Manager自我管理的功能、或是透過雲端分層管理的「即服務」技術ONTAP ONTAP FabricPool 。Cloud Tiering利用FabricPool 先進的自動化程序、在ONTAP 整個過程中、無論是在內部或雲端上、都能讓您更清楚地瞭解及控制混合雲和多雲端部署之間的分層作業。
...
若要取得預估、只要將 ONTAP 叢集新增至 BlueXP 、然後透過「雲端分層叢集」頁面進行檢查即可。按一下 * 計算叢集的潛在分層節約 * 以啟動 https://bluexp.netapp.com/cloud-tiering-service-tco["雲端分層 TCO 計算機"^] 看看您可以省下多少錢。
...
如果您有 Cloud Volumes ONTAP 系統、您可以在「雲端分層叢集」頁面找到這些系統、以便完整檢視混合雲基礎架構中的資料分層。不過Cloud Volumes ONTAP 、從Cloud Tiering讀取的是隻讀的系統。您無法從 Cloud Volumes ONTAP Cloud Tiering 設定資料分層功能。 https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-tiering.html["您可以在 BlueXP 的工作環境中設定 Cloud Volumes ONTAP 系統的分層"^]。
...
=== 我可以使用其他S3相容的物件儲存服務、例如MinIO、搭配雲端分層嗎？
