...
如果您達到BYOL容量限制、新的冷資料分層作業就會停止、而先前所有階層式資料仍可存取。如果您訂閱了「_Cloud Manager -部署與管理雲端資料服務」的PAYGO市場訂閱、新的Cold資料將繼續分層至物件式儲存設備、而且您將依每次使用付費。
...
也支援使用「鏡射鏡射」組態和「鏡射」組態的叢集FabricPool 、但分層組態必須使用System Manager或the irised CLI來完成。MetroCluster ONTAP
...
在典型組態中、整個叢集有一個物件存放區。自2022年8月起、您可以使用*進階設定*頁面來新增叢集的其他物件存放區、然後將不同的物件存放區附加到不同的集合體。
