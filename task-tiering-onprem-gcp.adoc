...
. 確認安裝連接器的 VPC 啟用下列連線：
+
** 透過連接埠 443 連接到雲端分層服務和 Google Cloud Storage 的 HTTPS 連線 (https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-set-up-networking-google.html#endpoints-contacted-for-day-to-day-operations["請參閱端點清單"^]）
