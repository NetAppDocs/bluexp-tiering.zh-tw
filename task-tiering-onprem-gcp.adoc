...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-connector-google.html["在GCP中部署Connector"^]
...
. 確認安裝連接器的 VPC 啟用下列連線：
+
** 透過連接埠 443 連接到 BlueXP 分層服務和 Google Cloud Storage 的 HTTPS 連線 (https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-set-up-networking-google.html#endpoints-contacted-for-day-to-day-operations["請參閱端點清單"^]）
...
NOTE: 如果您計畫設定 BlueXP 分層以使用低成本的儲存類別、讓階層式資料在一定天數後轉換至、則在 GCP 帳戶中設定儲存區時、您不得選擇任何生命週期規則。BlueXP 分層管理生命週期轉換。
...
.. * 儲存類別生命週期 * ： BlueXP 分層管理階層資料的生命週期轉換。資料是從_Standard_類別開始、但您可以建立規則、以便在特定天數後將資料移至其他類別。
+
