...
雲端分層授權也可與MetroCluster 採用「鏡射」和FabricPool 「鏡射鏡射」組態的叢集共享。必須使用System Manager或CLI來設定此功能、FabricPool ONTAP 但是 https://docs.netapp.com/us-en/cloud-manager-tiering/task-licensing-cloud-tiering.html#apply-cloud-tiering-licenses-to-clusters-in-special-configurations["這些叢集類型的授權使用Cloud Tiering完成。"]
